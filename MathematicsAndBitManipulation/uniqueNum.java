...
Commutative Property of XOR: XOR is a binary operation with the property that it doesn't matter in which order you perform it. In other words, for any two numbers a and b, a ^ b is the same as b ^ a. This property is known as the commutative property.

Example: 5 ^ 3 is the same as 3 ^ 5, and the result is 6 in both cases.

Associative Property of XOR: XOR is also associative, which means that when you have more than two numbers, the order in which you perform the XOR operation doesn't matter.
...
XORing a Number with Itself: When you XOR a number with itself, the result is always 0. This is a fundamental property of the XOR operation.
...
The only number that is not canceled out is the unique number that appears only once. XORing this unique number with 0 (the initial value) leaves it unchanged.
 */
